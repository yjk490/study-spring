...
//    @PostMapping("/login")
    public String login2(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        if (bindingResult.hasErrors()) {
            return "login/loginForm";
        }
    ...
    public String login3(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletRequest request) {
        ...
        // 세션이 있으면 이미 존재하는 세션을 반환하고, 없으면 새로 생성하여 반환
